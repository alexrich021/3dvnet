...
from mv3d.baselines.atlas.transforms import NYU40_COLORMAP
...
class TSDF():
    ...
    def load(cls, fname, voxel_types=None):
        ...
        with np.load(fname) as data:
            ...
            attributes     = {}
            if 'weight' in data:
                attributes['weight'] = torch.as_tensor(data['weight'])
            if 'tsdf_point_cloud' in data:
                attributes['tsdf_point_cloud'] = torch.as_tensor(data['tsdf_point_cloud'])
...
class TSDFFusion():
    ...
    def integrate(self, projection, depth, color=None, label=None):
        ...
        # tsdf distance
        dist = depth[py[valid], px[valid]] - pz[valid] # (n1) (where n1 is # of in valid voxels)
        dist = torch.clamp(dist / self.trunc_margin, max=1)
    ...
    def get_tsdf(self, label_name='instance'):
        ...
        attribute_vols = {'weight': self.weight_vol.view(nx,ny,nz)}
        if self.color_vol is not None:
            color_vol = self.color_vol.clone()
            color_vol[:, self.weight_vol>0]/=self.weight_vol[self.weight_vol>0]
            color_vol = color_vol.view(3,nx,ny,nz)
            attribute_vols['color'] = color_vol

            '''Generating colored point cloud'''
            # Marching cubes
            verts = measure.marching_cubes_lewiner(tsdf_vol.detach().cpu().numpy(), level=0)[0]
            verts_ind = np.round(verts).astype(int)
            verts = verts * self.voxel_size + self.origin.detach().cpu().numpy()
            ...
            pc = torch.from_numpy(pc)  # convert to tensor to avoid messing up tsdf save func
            attribute_vols['tsdf_point_cloud'] = pc
